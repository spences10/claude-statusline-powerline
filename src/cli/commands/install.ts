...
export function install_statusline(): void {
...
export function uninstall_statusline(): void {
	console.log('🗑️  Uninstalling claude-statusline-powerline...\n');

	try {
		// Remove statusline from Claude settings
		if (fs.existsSync(CLAUDE_SETTINGS_FILE)) {
			const settings_content = fs.readFileSync(
				CLAUDE_SETTINGS_FILE,
				'utf8',
			...
			if (settings.statusLine) {
				...
				console.log('✅ Removed statusline from Claude settings');
			}
		}
		...
		if (fs.existsSync(STATUSLINE_CONFIG_FILE)) {
			console.log(
				`Config file remains at: ${STATUSLINE_CONFIG_FILE}`,
			);
